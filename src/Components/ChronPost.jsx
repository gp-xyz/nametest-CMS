...
function PostContent({ post, withImage }) {
  return (
    <div className={`rounded shadow-2xl w-full h-64 relative border-slate-700 border-2 hover:border-yellow-300 hover:shadow-none`}>
      {withImage && <img src={post.img_url} className=" w-full h-full object-cover opacity-20" />}
      <div className="absolute inset-0 flex items-top justify-start p-3 m-3">
        <p className={`text-slate-700 text-center font-bold ${withImage ? 'text-3xl sm:text-3xl md:text-4xl blogtitle' : 'text-xl'}`}>{post.title.rendered}</p>
      </div>
